        ...
        <n-spin :show="loading">
          <n-form ref="formRef" label-placement="left" :label-width="80" :model="formValue" :rules="rules" :size="size">
            <n-form-item label="番号" path="carNo">
              <n-input
                v-model:value="formValue.carNo"
                :disabled="disabled"
                placeholder="入力しない場合は自動的に作成"
              />
            </n-form-item>
            ...
            <n-form-item label="写真" path="carPhoto">
              <n-space>
                <file-upload
                  ref="fileUploadRef"
                  accept="image/*"
            ...
            <n-form-item label="駐車場" path="carPark">
              <n-input
                v-model:value="formValue.carPark"
                placeholder="クリック駐車場を選択"
                readonly
...
const showModal = (row: MyModel.Car | undefined) => {
  ...
  if (row) {
    ...
    formValue.value.carPhoto = row?.carPhoto;
...
function renderFileUpload() {
  ...
  const { carPhoto } = formValue.value;
  if (carPhoto) {
    const file = {
      uid: 'logo',
      name: carPhoto,
      status: 'done',
      thumbUrl: `${envConfig.static}${carPhoto}`,
      url: `${envConfig.static}${carPhoto}`,
      fileUrl: carPhoto
    };
...
const onFinish = (files: any) => {
  if (!files || files.length <= 0) {
    formValue.value.carPhoto = undefined;
    return;
  }
  formValue.value.carPhoto = files[0].url;
};
  ...
  formRef.value?.validate(errors => {
    if (!errors) {
      const params: MyModel.Car = {
        ...
        carPhoto: formValue.value.carPhoto,
