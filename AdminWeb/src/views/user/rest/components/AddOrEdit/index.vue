			...
			<n-card size="huge" role="dialog" aria-modal="true">
				<n-spin :show="loading">
					<n-form ref="formRef" label-placement="left" :label-width="100" :model="formValue" :rules="rules"
						:size="size">
						<n-grid :cols="2"><n-gi :span="2">
								<n-form-item label="スタッフ" path="userName">
									<n-input-group>
										<n-input v-model:value="formValue.userName" readonly placeholder="クリックスタッフを選択"
											:disabled="!!formValue.id" @click="showStaff()"></n-input>
										<n-input-group-label>番号</n-input-group-label>
										<n-input v-model:value="formValue.userNo" :disabled="true" placeholder="ドライバーを選択して表示" />
									</n-input-group> </n-form-item></n-gi><n-gi :span="2">
								<n-form-item label="休暇タイプ" path="restType">
									<n-input v-model:value="formValue.restType" placeholder="クリックタイプを選択" readonly
										@click="showDict('user_rest_type')" /> </n-form-item></n-gi><n-gi>
								<n-form-item label="開始日時" path="startTime">
									<n-date-picker v-model:value="formValue.startTime" type="date" format="yyyy/MM/dd"
										clearable /></n-form-item></n-gi><n-gi>
								<n-form-item label="終了日時" path="endTime">
									<n-date-picker v-model:value="formValue.endTime" type="date" format="yyyy/MM/dd"
										clearable /></n-form-item></n-gi><n-gi :span="2">
