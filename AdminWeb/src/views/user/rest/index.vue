	...
	<div class="h-max">
		<n-space :vertical="true">
					...
					<n-form :inline="!isMobile" :label-width="100" label-placement="left">
						<n-form-item label="キーワード">
							<n-input v-model:value="searchParams.keyword" style="min-width: 30%" type="text"
								placeholder="スタッフ番号/名前/備考" clearable />
						</n-form-item>
			...
			<n-space>
				...
				<file-upload ref="fileUploadRef" class="ml-2" accept=".csv,.xls,.xlsx" action="/api/user/batchRest/"
					list-type="text" file-key="files/user" btn-text="一括登録" :max="999" :show-file-list="false"
					@finish="onFinish" />
			</n-space>
			<loading-empty-wrapper :style="{ height: hightRef + 'px' }" :loading="loading" :empty="empty">
				<n-data-table size="small" remote bordered :columns="columns" :data="dataSource" :pagination="pagination"
					:scroll-x="1000" :single-line="false" :flex-height="true" :style="{ height: hightRef + 'px' }"
