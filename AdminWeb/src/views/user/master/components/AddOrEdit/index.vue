            ...
            <n-grid :cols="2">
              <n-grid-item>
                <n-form-item label="番号" path="userNo">
                  <n-input v-model:value="formValue.userNo" :disabled="disabled" />
                </n-form-item>
              </n-grid-item>
              <n-grid-item>
                <n-form-item label="氏名" path="userName">
                  <n-input v-model:value="formValue.userName" :disabled="disabledRole" />
                </n-form-item>
              </n-grid-item>
              <n-grid-item>
                <n-form-item label="役職" path="selUserRoles" required>
                  <n-select
              ...
              <n-grid-item>
                <n-form-item label="Wechat" path="Wechat">
                  <n-input v-model:value="formValue.userWechat" />
                </n-form-item>
              </n-grid-item>
              <n-grid-item>
                <n-form-item label="Line" path="Wechat">
                  <n-input v-model:value="formValue.userLine" />
                </n-form-item>
              </n-grid-item>
              <n-grid-item>
                <n-form-item label="WhatsApp" path="Wechat">
                  <n-input v-model:value="formValue.userWhatsApp" />
                </n-form-item>
              </n-grid-item>
              <n-grid-item>
...
const showModal = (row: MyModel.User | undefined) => {
  ...
  if (row) {
    ...
    formValue.value.userLine = row?.userLine;
    formValue.value.userWechat = row?.userWechat;
    formValue.value.userWhatsApp = row?.userWhatsApp;
...
const handleValidateClick = (e: MouseEvent) => {
  ...
  formRef.value?.validate(errors => {
    if (!errors) {
      if (formValue.value.selUserRoles.length <= 0) {
        message.warning('役職を選択してください');
        return;
      }
      const params: MyModel.User = {
        ...
        userLine: formValue.value.userLine,
        userWechat: formValue.value.userWechat,
        userWhatsApp: formValue.value.userWhatsApp
      };
