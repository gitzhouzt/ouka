      ...
      <n-result status="info" size="small" description="以下の情報削除を行います。">
        <template #footer>
          <n-descriptions size="small" label-placement="left" bordered :column="1" :label-style="{ width: '20%' }">
            <n-descriptions-item label="表示">
              <n-space class="flex items-center">
                {{ props.model?.itemName }}
              </n-space>
            </n-descriptions-item>
            <n-descriptions-item label="コード">
              <n-space class="flex items-center">
                {{ props.model?.itemCode }}
              </n-space>
            </n-descriptions-item>
          </n-descriptions>
...
const props = defineProps({
  model: {
    type: Object as PropType<MyModel.DictItem | null>,
    default: () => {
      return null;
    }
  }
...
const urls = {
  delete: '/base/dictItem/deletePhysics'
};
const next = () => {
