      ...
      <n-result status="info" size="small" description="以下の情報削除を行います。">
        <template #footer>
          <n-descriptions size="small" label-placement="left" bordered :column="1" :label-style="{ width: '20%' }">
            <n-descriptions-item label="ユーザー">
              <n-space class="flex items-center">
                <div class="flex items-center">
                  <Icon icon="material-symbols:person" /><span class="ml-4px">{{ props.model?.userName }}</span>
                </div>
                <div class="flex items-center">
                  <Icon icon="material-symbols:mail" /><span class="ml-4px">{{ props.model?.userEmail }}</span>
                </div>
                <div class="flex items-center">
                  <Icon icon="material-symbols:phone-enabled" /><span class="ml-4px">{{
                    `${props.model?.countryNum ?? '-'} -  ${props.model?.userPhone ?? '-'}`
                  }}</span>
                </div>
              </n-space>
            </n-descriptions-item>
...
const props = defineProps({
  model: {
    type: Object as PropType<MyModel.User | null>,
    default: () => {
      return null;
    }
  }
