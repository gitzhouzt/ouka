...
import { EnumActionStepModule } from '@/enum';
...
const modelRef = ref<MyModel.OwnerRequest | null>(null);
const showModalRef = ref<boolean | undefined>(false);
const showModal = (row: MyModel.OwnerRequest | null) => {
...
interface StepModule {
  key: number;
  label: EnumActionStepModule;
  component: Component;
}

const modules: StepModule[] = [
  { key: 1, label: EnumActionStepModule['del-confirmed'], component: DelConfirm },
  { key: 2, label: EnumActionStepModule['del-success'], component: DelSuccess }
];
