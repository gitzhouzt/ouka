<template>
	<div class="h-max">
		<n-space :vertical="true">
			...
			<n-space>
				<n-button type="primary" @click="handleDictAdd('分類登録')">分類登録</n-button>
				<file-upload ref="fileUploadRef" class="ml-2" accept=".csv,.xls" action="/api/base/dictItem/batch/"
					list-type="text" file-key="files/dict" btn-text="一括登録" :max="999" :show-file-list="false"
					@finish="onFinish" />
			</n-space>
			<n-grid :cols="2">
				<n-grid-item>
		...
		<item-add-or-edit ref="itemAoeModal" @close="getItem(dictCode)" />
		<dict-del-action ref="dictDelModal" @close="searchQuery" />
		<item-del-action ref="itemDelModal" @close="getItem(dictCode)" />
	</div>
</template>
...
import { ItemAddOrEdit, DictAddOrEdit, DictDelAction, ItemDelAction } from './components';
...
const dictAoeModal = ref<any>(null);
const handleDictAdd = (title: string, row?: MyModel.Dict) => {
	dictAoeModal.value.setTitle(title);
	dictAoeModal.value.showModal(row);
};
...
const dictDelModal = ref<any>(null);
const handleDictDel = (title: string, row?: MyModel.Dict) => {
	dictDelModal.value.setTitle(title);
	dictDelModal.value.showModal(row);
};
		...
		render(row) {
			const editOption = h(
				NButton,
				{
					...
					size: 'small',
					type: 'info',
					onClick: () => handleDictAdd('データ編集', row)
				},
				{ default: () => '編集' }
			);
			return editOption;
