<template>
	<n-descriptions :size="size" label-placement="left" bordered :column="3" :label-style="labelStyle">
		...
		<n-descriptions-item label="ドライバー">
			<n-space><span :class="`${props.model?.driverName ?? 'text-red'}`">{{ props.model?.driverName ?? '未定' }}</span>
		...
		<template v-if="
			EnumOrderType[props.model?.orderType ?? ''] === EnumOrderType.Airport_S ||
			EnumOrderType[props.model?.orderType ?? ''] === EnumOrderType.Airport_Y
		">
			<n-descriptions-item label="空港">
				<n-space>
					<div class="flex items-center">
						{{
							props.model?.airport
								? props.model?.airport && props.model?.terminal
									? `${props.model?.airport} / ${props.model?.terminal}`
									: '-'
								: '-'
						}}
		...
		<n-descriptions-item label="終了日時">
			<n-space><span>{{ `${props.model?.endTime ?? '-'}` }}</span>
			</n-space>
		</n-descriptions-item>
		...
		<n-descriptions-item label="基本料金">
			<n-space>
				<div class="flex items-center">
					<span>{{ `${props.model?.currencyAmount} ${props.model?.currency}` }}</span>
				</div>
			</n-space>
		</n-descriptions-item>
		<n-descriptions-item label="转换日元">
			<n-space>
				<div class="flex items-center">
					<span class="">{{ `${props.model?.amount ?? 0} 円` }}</span>
				</div>
			</n-space>
		</n-descriptions-item>
