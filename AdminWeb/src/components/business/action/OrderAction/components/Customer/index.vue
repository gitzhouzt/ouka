				...
				<n-grid :cols="2">
					...
					<n-gi>
						<n-form-item label="都道府県&nbsp;&nbsp;" path="city">
							<n-input v-model:value="formValue.city" placeholder="クリック都道府県を選択" readonly
								@click="showDict('order_city', '')" /> </n-form-item></n-gi>
					<n-gi>
						<n-form-item label="责任者" path="sellerName">
							<n-input-group>
								<n-input v-model:value="formValue.sellerName" readonly placeholder="クリック责任者を選択"
									@click="showStaff()"></n-input>
							</n-input-group> </n-form-item></n-gi>
					<n-gi>
						<n-form-item label="お客様名" path="customerName">
							<n-input v-model:value="formValue.customerName" /></n-form-item></n-gi>
					<n-gi>
						<n-form-item label="請求先&nbsp;&nbsp;" path="billingAddress">
							<n-input v-model:value="formValue.billingAddress" /></n-form-item></n-gi>
					<n-gi>
						<n-form-item label="連絡方法①" path="contactMethod1">
							<n-input v-model:value="formValue.contactMethod1" placeholder="クリック連絡方法を選択" readonly
								@click="showDict('order_contact_method', '1')" /> </n-form-item></n-gi>
					<n-gi>
						<n-form-item label="連絡内容" path="contactContent1">
							<n-input v-model:value="formValue.contactContent1" @change="value => {
									onChange(value, '1');
								}
								" /></n-form-item></n-gi>
					<n-gi>
						<n-form-item label="連絡方法②&nbsp;&nbsp;">
							<n-input v-model:value="formValue.contactMethod2" placeholder="クリック連絡方法を選択" readonly
								@click="showDict('order_contact_method', '2')" /> </n-form-item></n-gi>
					<n-gi>
						<n-form-item label="連絡内容&nbsp;&nbsp;">
							<n-input v-model:value="formValue.contactContent2" @change="value => {
									onChange(value, '2');
								}
								" /></n-form-item></n-gi>
					<n-gi>
						<n-form-item label="連絡方法③&nbsp;&nbsp;">
							<n-input v-model:value="formValue.contactMethod3" placeholder="クリック連絡方法を選択" readonly
								@click="showDict('order_contact_method', '3')" /> </n-form-item></n-gi>
					<n-gi>
						<n-form-item label="連絡内容&nbsp;&nbsp;">
							<n-input v-model:value="formValue.contactContent3" @change="value => {
									onChange(value, '3');
								}
								" /></n-form-item></n-gi>
				</n-grid>
