...
import { REGEXP_PWD, REGEXP_CODE_SIX, REGEXP_EMAIL, REGEXP_PHONE, REGEXP_POST } from '@/config';

/** form rules */
interface CustomFormRules {
  /** email */
  mail: FormItemRule[];
  mail_option: FormItemRule[];
  /** phone */
  phone: FormItemRule[];
  phone_option: FormItemRule[];
  /** password */
  pwd: FormItemRule[];
  /** verification code */
  code: FormItemRule[];
...
export const formRules: CustomFormRules = {
  mail: [
    ...
    { pattern: REGEXP_EMAIL, message: '有効なメールアドレスを入力してください', trigger: 'input' }
  ],
  mail_option: [
  ...
  phone: [
    ...
    { pattern: REGEXP_PHONE, message: '半角数字電話番号を入力ください。', trigger: 'input' }
  ],
  phone_option: [
  ...
  code: [
    ...
    { pattern: REGEXP_CODE_SIX, message: '有効な確認コードを入力ください', trigger: 'input' }
  ],
  post: [
    { required: false, message: '郵便番号を入力してください' },
    { pattern: REGEXP_POST, message: '有効な郵便番号を入力ください', trigger: 'input' }
  ]
};
...
export function getConfirmPwdRule(pwd: Ref<string>) {
  const confirmPwdRule: FormItemRule[] = [
    { required: true, message: '確認パスワードを入力してください' },
    {
      ...
      message: '新しいパスワードが確認用に入力されたものと異なります。入力しなおしてください。',
      trigger: 'input'
    }
  ];
