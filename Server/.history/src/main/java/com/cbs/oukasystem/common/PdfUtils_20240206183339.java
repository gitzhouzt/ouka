...
public class PdfUtils {
    ...
    public void sigFieldWithDivAppearance(String dest) throws IOException, InterruptedIOException {
        try (Document document = new Document(new PdfDocument(new PdfWriter(dest)))) {
            Div div = new Div();
            div.add(new Paragraph("Paragraph inside
                            .setBorder(new DashedBorder(ColorConstants.RED, 1)).setBackgroundColor(ColorConstants.PINK
                            Container = new Div();
                            ainer.setProperty(Property.FLEX_WRAP, FlexWrapPropertyValue.WRAP);
                            ainer.setProperty(Property.FLEX_DIRECTION, FlexDirectionPropertyValue.ROW_REVERSE);
                            ainer.setPropert
                    y(Property.ALIGN_ITEMS, AlignmentPropertyValue.CENTER);
