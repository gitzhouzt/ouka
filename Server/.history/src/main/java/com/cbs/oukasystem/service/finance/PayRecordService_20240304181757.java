...
public class PayRecordService {
    ...
    public Specification<PayRecordEntity> Search(QueryPayRecordVO qVo) {
        return new Specification<PayRecordEntity>() {
            ...
            public Predicate toPredicate(Root<PayRecordEntity> root, CriteriaQuery<?> query, CriteriaBuilder cb) {
                ...
                if (null != qVo.getPayMethod() && qVo.getPayMethod().length > 0) {
                    Predicate[] method = new Predicate[qVo.getPayMethod().length];
                    int index = 0;
                    for (String m : qVo.getPayMethod()) {
                        method[index] = cb.equal(root.get("payMethod"), m);
                        index++;
                    }
                    predicate.add(cb.or(method));
                }
    ...
    public Boolean settlement(List<String> ids, String payMethod, String payMethodCode, String bank) {
        repository.settlementByIds(ids, payMethod, payMethodCode, bank, 1);
        return true;
    }
     ...
     */
    public Boolean settlement(List<String> ids, String payMethod, String payMethodCode, String bank) {
        repository.settlementByIds(ids, payMethod, payMethodCode, bank, 2);
        return true;
    }
