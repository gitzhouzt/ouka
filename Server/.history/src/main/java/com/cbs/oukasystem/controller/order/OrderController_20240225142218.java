...
public class OrderController {
    ...
    public ResultVO<PayRecordVO> insertOrUpdate(@Validated @RequestBody IUPayRecordVO iuPayVO) {
        return ResultVO.success(service.pay(iuPayVO));
    }

    @ResponseBody
    @Operation(summary = "confirmPay 料金記録確認")
    @PostMapping("/confirmPay/{id}")
    public ResultVO<String> confirmPay(@PathVariable @Parameter(required = true, description = "id") String id) {
        return ResultVO.success(service.confirmPay(id));
    }
