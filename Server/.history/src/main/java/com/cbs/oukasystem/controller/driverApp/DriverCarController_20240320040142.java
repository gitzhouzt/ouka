...
public class DriverCarController {
    ...
    public ResultVO<CarDetailsVO> details(@PathVariable @Parameter(required = true, description = "id") String id) {
    ...
    @ResponseBody
    @Operation(summary = "report Order - 立替申请")
    @PostMapping(value = "/report")
    @Parameters(@Parameter(name = "iuCarDamageVO", description = "立替申请", required = true))
    public ResultVO<CarDamageVO> report(@Validated @RequestBody IUCarDamageVO iuCarDamageVO) {
        return ResultVO.success(service.report(iuCarDamageVO));
    }
