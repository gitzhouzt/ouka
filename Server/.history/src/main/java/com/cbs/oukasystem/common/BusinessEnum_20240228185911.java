...
public final class BusinessEnum {
    ...
    public enum EnumRecordType {
        ...
        InEarnings("In_Earnings", "入金(売上)"),
        InAdvance("InAdvance", "入金(立替)"),
        OutAdvance("OutAdvance", "出金(立替)"),
        OutCost("OutCost", "出金(コスト)");

        private String code;
        private String message;

        private EnumRecordType(String code, String message) {
            this.code = code;
            this.message = message;
        }
        ...
        @JsonCreator
        public static EnumRecordType forName(String name) {
            for (EnumRecordType c : values()) {
                if (c.name().equals(name)) { // change accordingly
                    return c;
                }
            }
