...
public class CarCheckController {
    ...
    public ResultVO<CarCheckVO> addOrEdit(@Validated @RequestBody IUCarCheckVO iuCarCheckVO) {
        return ResultVO.success(service.addOrEdit(iuCarCheckVO));
    }

    @ResponseBody
    @Operation(summary = "setLastCheckDate CarCheck - 追加・編集車両点検")
    @PostMapping(value = "/setLastCheckDate")
    @Parameters(@Parameter(name = "iuCarCheckVO", description = "追加・編集車両点検", required = true))
    public ResultVO<CarCheckVO> setLastCheckDate(@Validated @RequestBody IUCarCheckVO iuCarCheckVO) {
        return ResultVO.success(service.setLastCheckDate(iuCarCheckVO));
    }
