...
public class DriverUserController {
    ...
    @ResponseBody
    @Operation(summary = "写日记 - 追加・編集 ユーザー")
    @PostMapping(value = "/writeRecord")
    @Parameters(@Parameter(name = "writeRecord", description = "追加・編集 ユーザー", required = true))
    public ResultVO<LogRecordVO> writeRecord(@Validated @RequestBody IULogRecordVO iuLogRecordVO) {
        return ResultVO.success(service.writeLogRecord(iuLogRecordVO));
    }

    @ResponseBody
    @Operation(summary = "LogDetail -日记情報")
    @GetMapping("/log/details/{logId}")
    public ResultVO<LogDetailsVO> getLogDetails(
            @PathVariable @Parameter(required = true, description = "logId") String logId) {
        return ResultVO.success(service.getLogDetails(logId));
    }
}
